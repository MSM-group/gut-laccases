...
library(pacman)
pacman::p_load("ape", "tidyverse", "ggtree", "janitor", "ggnewscale", "rentrez", "scales", "phylosignal", "phylobase", "xml2", "ursa", "colorspace", "RColorBrewer")
...
rownames(treetmap_dat) <- dplyr::filter(tree_plot_2@data, isTip == TRUE) %>%
  pull(label)
set.seed(67)
#Option D: instead of drawing many colours from one auto-generated palette
#(which tends to bunch up several similar reds/pinks, as seen in fig3_a.R),
#pool several small, human-curated, colourblind-conscious qualitative palettes
#that were each already designed for internal distinctness, then greedily pick
#the n_family most mutually distinct colours (in perceptual Lab distance) from
#that combined pool, filtered to avoid the reserved grey90/black lightness/chroma
n_family <- na.omit(metadat_3$family) %>%
  unique() %>%
  length()
mixed_candidates <- unique(toupper(c(
  grDevices::palette.colors(8, "Okabe-Ito")[-1],
  RColorBrewer::brewer.pal(8, "Dark2"),
  RColorBrewer::brewer.pal(9, "Set1"),
  RColorBrewer::brewer.pal(12, "Paired"),
  RColorBrewer::brewer.pal(8, "Accent")
)))
mixed_lab <- colorspace::coords(methods::as(colorspace::hex2RGB(mixed_candidates), "LAB"))
mixed_chroma <- sqrt(mixed_lab[, 2]^2 + mixed_lab[, 3]^2)
mixed_filtered <- mixed_candidates[mixed_lab[, 1] > 15 & mixed_lab[, 1] < 85 & mixed_chroma > 20]
#Greedily pick colours one at a time, always adding the candidate that is
#farthest (in perceptual Lab distance) from every colour already picked, so
#similar-looking candidates don't all get selected together
select_distinct_colors <- function(candidates, n) {
  lab <- colorspace::coords(methods::as(colorspace::hex2RGB(candidates), "LAB"))
  dist_mat <- as.matrix(dist(lab))
  selected <- 1
  for (i in 2:n) {
    min_dist_to_selected <- apply(dist_mat[, selected, drop = FALSE], 1, min)
    min_dist_to_selected[selected] <- -Inf
    selected <- c(selected, which.max(min_dist_to_selected))
  }
  candidates[selected]
}
family_palette <- c(select_distinct_colors(mixed_filtered, n_family), "grey90", "black")
...
tree_plot_2@data <- tree_plot_2@data %>%
  ...
  dplyr::mutate(family = family %>%
                ...
                tested = case_when(label %in% c("UrtQLP2E9X6p397RBN4r39_2", "NnqfqxssYuBZDZgVVf4AGr_9", "jj54KTrq2tPoS3oB95CWpb_3", "BU5axyDn7aeu7yGoESJCGb_10", "PvSv2mwhn8efmYDPquH3h3_2", "mMCO", "aMCO", "MCO1") ~ "Yes",
                                   TRUE ~ "No"))
tree_plot_2 <- tree_plot_2 + scale_color_discrete(palette = family_palette, labels = format_labels) +
...
ggplot2::ggsave("plots/fig3_d.jpg", tree_plot_3, device = "jpeg", units = "in", width = 7, height = 7, dpi = 300)
