...
#Option C: families represented by only a handful of tips are visually
#indistinguishable from one another regardless of palette. Fold any family
#below min_tips into a single "Other family" bucket, and keep a lookup table
#of which tip belongs to which of those folded-in families.
min_tips <- 3
family_counts <- metadat_3 %>%
  dplyr::count(family) %>%
  dplyr::filter(!is.na(family))
rare_families <- family_counts %>%
  dplyr::filter(n < min_tips) %>%
  dplyr::pull(family)
other_family_lookup <- metadat_3 %>%
  dplyr::filter(family %in% rare_families) %>%
  dplyr::select(duplicate_of, ncbi_taxonomy, family)
readr::write_csv(other_family_lookup, "plots/fig3_c_other_family_lookup.csv")
...
rownames(treetmap_dat) <- dplyr::filter(tree_plot_2@data, isTip == TRUE) %>%
  pull(label)
set.seed(67)
n_family <- family_counts %>%
  dplyr::filter(!family %in% rare_families) %>%
  nrow()
n_family <- n_family + 1 #account for the added "Other family" bucket
family_palette <- c(grDevices::palette.colors(n = n_family + 1, palette = "Okabe-Ito")[-1], "grey90", "black")
format_labels <- function(x) {
  lapply(x, function(l) {
    if (l %in% c("No family assignment", "Reference laccase", "Other family")) {
      l
    } else {
      bquote(italic(.(l)))
    }
...
tree_plot_2@data <- tree_plot_2@data %>%
  dplyr::mutate(family = dplyr::case_when(is.na(family) & !is.na(ncbi_taxonomy) & isTip == TRUE ~ "No family assignment",
                                          ...
                                          family %in% rare_families ~ "Other family",
                                          TRUE ~ family)) %>%
  dplyr::arrange(family) %>%
  dplyr::mutate(family = family %>%
                  as_factor() %>%
                  fct_relevel("Other family", after = Inf) %>%
...
ggplot2::ggsave("plots/fig3_c.jpg", tree_plot_3, device = "jpeg", units = "in", width = 7, height = 7, dpi = 300)
