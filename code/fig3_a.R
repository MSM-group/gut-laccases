...
library(pacman)
pacman::p_load("ape", "tidyverse", "ggtree", "janitor", "ggnewscale", "rentrez", "scales", "phylosignal", "phylobase", "xml2", "ursa", "pals", "colorspace")
...
rownames(treetmap_dat) <- dplyr::filter(tree_plot_2@data, isTip == TRUE) %>%
  pull(label)
set.seed(67)
#Option A: swap the cubehelix palette (a sequential palette, not designed for
#categorical distinctness) for a maximally-distinct qualitative palette, filtered
#to avoid lightness values that would collide with the reserved grey90/black slots
n_family <- na.omit(metadat_3$family) %>%
  unique() %>%
  length()
glasbey_candidates <- as.character(pals::glasbey(32))
glasbey_lab <- colorspace::coords(methods::as(colorspace::hex2RGB(glasbey_candidates), "LAB"))
glasbey_filtered <- glasbey_candidates[glasbey_lab[, 1] > 15 & glasbey_lab[, 1] < 85]
...
#similar-looking candidates (e.g. several reds) don't all get selected together
...
family_palette <- c(select_distinct_colors(glasbey_filtered, n_family), "grey90", "black")
...
ggplot2::ggsave("plots/fig3_a.jpg", tree_plot_3, device = "jpeg", units = "in", width = 7, height = 7, dpi = 300)
