...
rownames(treetmap_dat) <- dplyr::filter(tree_plot_2@data, isTip == TRUE) %>%
  pull(label)
set.seed(67)
#Option B: swap the cubehelix palette for a maximally-distinct qualitative palette
#(as in Option A) AND additionally map point shape to family, so family identity
#is redundantly encoded by both colour and shape. This frees up the shape
#aesthetic, so "Experimentally validated" is now shown via point size instead.
...
family_palette <- c(glasbey_filtered[1:n_family], "grey90", "black")
family_shapes <- 0:(n_family + 1)
...
tree_plot_2 <- tree_plot_2 + scale_color_discrete(palette = family_palette, labels = format_labels) +
  scale_shape_manual(values = family_shapes, labels = format_labels) +
  scale_size_manual(values = c("No" = 1.5, "Yes" = 3)) +
  ggtree::geom_tippoint(aes(x = x, y = y, color = family, shape = family, size = tested)) +
  new_scale_fill() +
  theme(legend.text = element_text(size = 8),
        legend.title = element_text(size = 8)) +
  labs(color = "Kraken2 family assignment", shape = "Kraken2 family assignment", size = "Experimentally validated")
...
ggplot2::ggsave("plots/fig3_b.jpg", tree_plot_3, device = "jpeg", units = "in", width = 7, height = 7, dpi = 300)
